...
const Footer: React.FC = () => {
  ...
  // Fetch cached IPs when dialog opens
  useEffect(() => {
    if (isDialogOpen) {
      const fetchCachedIPs = async () => {
        setIsLoading(true);
        try {
          // Fetch cached IPs from Redis
          const response = await axios.get("/api/recent-ips");
          setIpDetails(response.data);
          setPage(1); // Reset page for new dialog session
          // Check if there are more IPs in MongoDB
  ...
  // Helper to get total IP count from MongoDB
  const UniqueViewsCount = async () => {
    try {
      const response = await axios.get("/api/unique-views");
  ...
  const loadMoreIPs = async () => {
    ...
    try {
      ...
      setIpDetails((prev) => [...prev, ...response.data.ipDetails]);
      setPage((prev) => prev + 1);
    ...
    <footer className=" flex items-center justify-between bg-transparent w-full mx-auto p-4 dark:bg-transparent shadow-[2px_4px_16px_0px_rgba(248,248,248,0.06)_inset] group">
      <div className="start flex items-center justify-center gap-2">
        <a href="https://www.instagram.com/hv__in/" target="_blank" rel="noopener noreferrer">
          <svg
            className="m-2 w-5 h-5 text-blue-500 transition-transform duration-200 hover:scale-125"
        ...
        <a href="https://discord.com/users/hvin8" target="_blank" rel="noopener noreferrer">
          <svg
            className="m-2 w-6 h-6 text-blue-500 transition-transform duration-200 hover:scale-125"
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 16 16"
            fill="currentColor"
          >
        ...
        <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
          ...
          <DialogContent className="sm:max-w-[500px] max-h-[80vh] overflow-y-auto">
            <DialogHeader>
              ...
              <DialogDescription>
                List of unique visitor IPs and their details.
              </DialogDescription>
            </DialogHeader>
            ...
            <div className="max-h-[50vh] overflow-y-auto space-y-2">
              {ipDetails.length > 0 ? (
                ipDetails
                  ...
                  .map((ipInfo: any, index: number) => (
                    <div key={index} className="border p-2 rounded-md">
                      <p>
                        <strong>Time:</strong> {dayjs(ipInfo.createdAt).fromNow()}
                      </p>
                      <p>
                      ...
                      <p>
                        <strong>Location:</strong> {ipInfo.location || "N/A"}
                      </p>
                      <p>
                        <strong>Org:</strong> {ipInfo.org || "N/A"}
                      </p>
                      <p>
                        <strong>Postal Code:</strong> {ipInfo.postalCode || "N/A"}
                      </p>
                      <p>
                ...
                <p className="text-gray-500">{isLoading ? "Loading..." : "No IPs found"}</p>
              )}
            </div>
            ...
            <DialogFooter className="flex justify-between">
              ...
              {hasMore && (
                <button
                  onClick={loadMoreIPs}
                  disabled={isLoading}
                  className={`text-sm text-blue-500 hover:text-blue-600 hover:cursor-pointer ${isLoading ? "opacity-50" : ""}`}
                >
      ...
      <div className="end flex items-center justify-center">
        <a href="mailto:hvinprimary@gmail.com">
          <p className="text-xs text-center text-blue-500 hover:text-blue-600 transition-transform duration-200 hover:scale-115">
            Collaborate with me!
          </p>
        </a>
      </div>
